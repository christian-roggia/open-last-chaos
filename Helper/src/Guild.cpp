...
void CGuildList::sendlist(CDescriptor *desc)
{
    CGuild *t_guild;
    CGuild *p_guild;
    CNetMsg v_netmsg; // [sp+20h] [bp-14h]@1

    //CNetMsg::CNetMsg(&v_netmsg);
    for ( p_guild = this->Guild; p_guild; p_guild = p_guild->nextguild() )
    {
        HelperGuildCreateNotifyMsg(v_netmsg, p_guild);
        if ( desc )
            desc->WriteToOutput(v_netmsg);
        HelperGuildMemberListMsg(v_netmsg, p_guild);
        if ( desc )
            desc->WriteToOutput(v_netmsg);
        if ( p_guild->leftchild() )
        {
            ...
            if ( desc )
                desc->WriteToOutput(v_netmsg);
            t_guild = p_guild->leftchild();
            HelperGuildMemberListMsg(v_netmsg, t_guild);
            if ( desc )
                desc->WriteToOutput(v_netmsg);
            t_guild = p_guild->leftchild();
            if ( t_guild->leftchild() )
