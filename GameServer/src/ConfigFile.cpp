...
bool CConfigFile::Load(char const *data, int size)
{
    char group[1024], name[1024], value[1024];
    int counter = 0, c_group = 0, c_name = 0, c_value = 0;
...
bool CConfigFile::Add(char const *group, char const *name, char const *value)
{
    ...
    this->Configs[this->Count] = new CConfigFileData();
    this->Configs[this->Count++]->Set(group, name, value);
    this->Sort();
...
{
    if(this->Count > 1)
        qsort(this->Configs, this->Count, 4, CConfigFile::Comp);
}
...
char *CConfigFile::Find(char const *group, char const *name)
{
    ...
    src_data_ptr = bsearch(&c_data, this->Configs, this->Count, 4, CConfigFile::Comp);
