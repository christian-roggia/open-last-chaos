...
private:
    char p_log[0x2800];         // +0
    char p_temp_buffer[0x2800]; // +10240
public:
    CLogBuffer(void)                                        { this->Init(); }

    void __ls(char const *log)                              { this->appendLog(log); }
    void __ls(int log)                                      { __ltoa(log, this->p_temp_buffer); this->appendLog(this->p_temp_buffer); }
    void __ls(long long log)                                { __ltoa(log, this->p_temp_buffer); this->appendLog(this->p_temp_buffer); }
    void __ls(float log)                                    { __ftoa(log, this->p_temp_buffer, 4); this->appendLog(this->p_temp_buffer); }
...
CLogBuffer &delim(CLogBuffer &log_buffer);
