...
bool CDBCmd::GetRec(int m_field, char *m_value, bool m_unk)
{
    ...
    if(v7)
    {
        ...
        if(m_unk)
        {
            while(*v5 && *(short *)(_ctype_b + 2 * *v5) & 0x2000)
            ++v5;
        }
        ...
        if(m_unk)
        {
            for(i = &m_value[strlen(m_value) - 1]; i > m_value && *i && *(short *)(_ctype_b + 2 * *i) & 0x2000; --i);
            i[1] = 0;
        }
...
signed int CDBCmd::FindField(char const *p_fields_to_find)
{
    int i = 0;
