...
char *__ltoa(int m_number, char *m_dest)
{
    long i = 0, j = 0;
    char m_temp_buffer[128]; // WHY 128?
    ...
    *m_dest = 0;
    ...
    if(!m_number)
    {
        strcpy(m_dest, "0"); // <--------------------- MAKE SENSE
        //strcpy(m_dest, ""); // <-------------------- THIS NOT -> CHECK???
        return m_dest; // NOT SURE...
    }

    while(m_number)
    {
        m_temp_buffer[j++] = (m_number % 10 > 9 || m_number % 10 < 0) ? 48 : (m_number % 10 + 48);
        m_number /= 10;
    }
...
{
    int i; // [sp+40h] [bp-88h]@1
    int j; // [sp+44h] [bp-84h]@1
    ...
    *m_dest = 0;
    ...
    while(m_int64)
    {
        v6[j++] = (m_int64 % 10 > 9 || m_int64 % 10 < 0) ? 48 : (m_int64 % 10) + 48; // (_cmpdi2(v3, SHIDWORD(v3), 9, 0) > 1 || _cmpdi2(v3, SHIDWORD(v3), 0, 0) < 1)
        m_int64 /= 10;
    }
...
void TimeDiff(timeval *m_time1, timeval *m_time2, timeval *m_time3)
{
    if(m_time2->tv_sec >= m_time3->tv_sec)
    {
        if(m_time2->tv_sec == m_time3->tv_sec)
        {
            ...
            {
                m_time1->tv_sec = a_NullTime.tv_sec;
                m_time1->tv_usec = a_NullTime.tv_usec;
            }
        }
        else
        {
...
{
    while(m_string && *m_string && ((*m_string == ' ' || *m_string) > 8 && *m_string <= 13)) // COMP ~
        ++m_string;
...
{
    for(int i = strlen(m_string) - 1; i >= 0 && ((m_string[i] == ' ' || m_string[i] > 8) && m_string[i] <= 13); ++i)
        m_string[i] = 0;
...
char *TrimString(char *m_string)
{
    ...
    if(i > 0)
    {
        for(; j <= (signed long)strlen(m_string) - i; ++j)
...
char *AnyOneArg(char *m_string, char *m_args, bool m_check)
{
    ...
    for(char *i = SkipSpaces(m_string); i && *i && *i != ' ' && (*i <= 8 || *i > 13); ++i)
    {
        if(m_check)
...
char *IntCat0(char *dest, int a2, int a3, bool a4)
{
    ...
    char src[128]; // [sp+18h] [bp-80h]@1
    int v10; // [sp+A8h] [bp+10h]@4

    v8 = strlen(__ltoa(a2, src));
    v7 = strlen(dest);
...
bool strinc(char *m_string, const char *m_to_find)
{
    ...
    if(m_string_len >= m_to_find_len)
    {
        ...
        {
            if(i > (signed long)(m_string_len - m_to_find_len))
                return false;
        }
