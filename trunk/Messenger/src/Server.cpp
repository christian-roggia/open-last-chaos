...
void CServer::ProcTimeout(CMsgListNode *msglistnode)
{
    ...
    int v8; // ST58_4@1
    int v9; // ST28_4@1
    float v30; // [sp+24h] [bp-C0h]@17
    float v31; // [sp+28h] [bp-BCh]@17
    int a10; // [sp+2Ch] [bp-B8h]@17
    int a9; // [sp+30h] [bp-B4h]@17
    int a8; // [sp+34h] [bp-B0h]@17
    char a7[76]; // [sp+4Ch] [bp-98h]@11
    int a6; // [sp+B0h] [bp-34h]@11
    CDescriptor *olddesc; // [sp+B4h] [bp-30h]@1
    CDescriptor *newdesc; // [sp+B8h] [bp-2Ch]@2
    CNetMsg *p_netmsg; // [sp+CCh] [bp-18h]@1
    unsigned __int8 v42; // [sp+D3h] [bp-11h]@1
    int a5; // [sp+D4h] [bp-10h]@1
    int a4; // [sp+D8h] [bp-Ch]@1
    int a3; // [sp+DCh] [bp-8h]@1
    int dest; // [sp+E0h] [bp-4h]@1
    ...
    p_netmsg->__rs(v42);
    v6 = v42;
    v7 = msglistnode->NetMsg.p_msg_type;
    v8 = a4;
    v9 = a3;
    g_gamelogbuffer.__ls(init("SYS_ERR"));
    ...
    g_gamelogbuffer.__ls(end);
    olddesc = this->NewDesc;
    do
    {
        newdesc = olddesc;
        if ( !olddesc )
            break;
        olddesc = newdesc->SubDesc;
    }
    while ( newdesc != msglistnode->desc );
    if ( newdesc )
    {
        if ( v42 == 6 )
        {
            p_netmsg->__rs(a7);
            CNetMsg v_netmsg;
            MsgrLogoutRepMsg(v_netmsg, dest, a3, a4, a5, 0, a7);
            if ( msglistnode->desc )
                msglistnode->desc->WriteToOutput(v_netmsg);
        ...
        else
        {
            if ( (signed int)v42 > 6 )
            {
                if ( v42 == 10 )
                {
                    ...
                    p_netmsg->__rs(v30);
                    CNetMsg v_netmsg;
                    MsgrGoZoneRepMsg(v_netmsg, dest, a3, a4, a5, 0, a6, a8, a9, a10, v31, v30);
                    if ( msglistnode->desc )
                        msglistnode->desc->WriteToOutput(v_netmsg);
                    //CNetMsg::_CNetMsg(&v_netmsg, 2);
                }
            ...
            else
            {
                if ( !v42 )
                {
                    ...
                    p_netmsg->__rs(a7);
                    CNetMsg v_netmsg;
                    MsgrWhisperNotfoundMsg(v_netmsg, dest, a3, a4, a5, a6, a7);
                    if ( msglistnode->desc )
                        msglistnode->desc->WriteToOutput(v_netmsg);
                    //CNetMsg::_CNetMsg(&v_netmsg, 2);
                }
